...
You can probably spot a trend just by looking at the sorted scores there: lots of high scores, a few low scores. But how best to summarize and communicate the frequencies? An ungrouped frequency table covering the observed range from 3 through 20 would need 18 rows. If we included every possible test score from 0 through 20, it would need 21. And quite a few of those rows would have a frequency of zero or one. Here are the first few:
...
1. Choose enough bins, given the range of the data, to reveal the shape of the distribution without simply recreating the raw list. Around ten bins is often a good target, but that isn't written in stone. In some cases more or fewer than ten may be better.
...
At the two ends, the conventional zero-frequency points likewise extend the shape beyond the range of scores we observed. None of this is necessarily wrong or misleading. The lines help us see and compare an overall shape, and we might deliberately treat that shape as a simple model of how frequencies change across the scale. But it partly reflects our choices about how to connect and extend the observed points, not just the literal frequencies we recorded.
