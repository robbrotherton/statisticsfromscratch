...
For a single set of scores, choosing between a histogram and a frequency polygon may largely be a matter of preference or convention. Comparing the histogram in @fig-histogram and the frequency polygon in @fig-freqpoly, which are both based on the same five frequencies, you can see how the formats emphasize slightly different things. A histogram makes the bins and their frequencies tangible, while a frequency polygon draws the eye toward the overall shape of the distribution, and the continuous transitions between the scores.
...
It's worth noticing that the straight lines joining the dots arguably imply something that wasn't literally in the data. A line segment between two plotted points interpolates frequencies between them, modeling a linear increase or decrease along the continuous scale between one bin's center and the next. @fig-freqpoly-labeled duplicates @fig-freqpoly, but it adds dashed guides at a score of 1.5. At that point, the line reaches a height of 4, visually implying a frequency of 4 for scores of 1.5. But there were no actual scores of 1.5 in this dataset. At the two ends, the conventional zero-frequency points extend the shape beyond the range of scores we observed. These help present the distribution as a single coherent shape, but again suggest frequencies between points that we might not have observed. I'm not saying this is wrong or misleading, but it should be done with awareness and intent, because it takes us from a more literal representation of the data we recorded to something more like a visual indication of how we are modeling the data.
