...
::: {.bc-figure-row .bc-figure-row-wide .bc-standardization-comparison-row}
...
To make an IQ-style scale, first set the new standard deviation to 15. Keep the grey raw-score ruler as a reference and <span class="bc-data-emphasis">multiply every number on the $z$ ruler by 15</span>. The mean remains 0, while the standard deviation becomes 15. The marked score changes from 2 to 30.
:::
...
Now <span class="bc-data-emphasis">add 100 to every number</span> on that lower ruler. Adding the same amount to every score changes the mean from 0 to 100 without changing the standard deviation of 15. The marked position that was labeled 30 becomes 130, while the grey raw-score ruler still labels it 80.
:::
...
For our simplified SAT-style scale, <span class="bc-data-emphasis">multiply every $z$-score by 100</span>. This changes the standard deviation from 1 to 100 while leaving the mean at 0. The marked $z$-score of 2 becomes 200.
:::
...
Finally, <span class="bc-data-emphasis">add 500 to every score</span>. The reporting mean becomes 500 and the standard deviation stays 100. The same position is now labeled 80 on the original ruler, 2 on the $z$ ruler, and 700 on the SAT-style ruler.
:::
::::
...
{{< interactive-figure reactionTimeDistribution makeDistributionGraph options='{"distribution":"normal","mean":284,"sd":50,"xDomain":[134,434],"xTicks":7,"style":"minimal","xAxis":true,"markers":{"at":159,"label":"X = 159","height":0.5,"color":"var(--bc-danger-color, #c63f3f)","dash":"6 4","strokeWidth":2.6},"animate":true,"labels":{"x":"Reaction time (ms)"}}' fallback="Normal curve of reaction times centered on 284 ms with standard deviation 50 ms; a vertical line marks Peter Parker's 159 ms far down in the lower tail." >}}
