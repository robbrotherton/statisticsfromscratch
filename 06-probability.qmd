...
::: {#fig-catan}
{{< interactive-figure catanBoardIntro makeCatanBoard options='{"mode":"choose","remember":"bc-catan-choice"}' fallback="A schematic Settlers of Catan board: nineteen hexagonal tiles in three terrain colors, each except the central desert carrying a number from 2 to 12. Settlements go at the corners where tiles meet." >}}
...
{{< interactive-figure
  catanBoardVerdict
  makeCatanBoard
  options='{"mode":"rated","rating":"empirical","linkTo":"probabilityDiceRoller","remember":"bc-catan-choice","showBoard":false}'
...
The probability of the second marble being white actually depends on how exactly we're doing our sampling. Specifically, it depends on whether we put the first marble back into the jar before we take the second one. This is the distinction between sampling with and without replacement.
...
What if the distribution isn't a little collection of ten blocks we can count? This is where the normal distribution makes a key advance possible.

We have already met some of its history. The curve we now call normal emerged from work on games of chance and was developed as a model for the small errors that accumulate when astronomers repeatedly measure the same thing [@stahl2006normal]. Then, in the nineteenth century, Adolphe Quetelet made a more surprising move. As we saw in [Chapter 3](03-central-tendency.qmd), he treated the chest measurements of 5,738 Scottish soldiers as if their deviations from the average soldier followed the same kind of pattern as astronomers' errors. The fit wasn't perfect, but the broader idea stuck: the same probability curve could be used as a model for variation among people, not only variation among repeated measurements [@gallagher2020quetelet].

Now, here's the insight that sets us up for much of the inferential statistics ahead. For a distribution in general, knowing only the mean and standard deviation isn't enough to determine probabilities. But a normal distribution is completely specified by its mean and standard deviation. If a population can be reasonably modelled by a normal distribution, we can determine the probability associated with any region of scores without recording and counting every individual score.

This is a real shift from counting the blocks above. A normal distribution is **continuous**: between any two possible values lie still more possible values. The probability of selecting any one exact value is therefore zero; probabilities belong to intervals, such as scores below a cutoff or between two values. That doesn't mean an exact value is impossible. It means that a single point has no width and therefore no area under a continuous curve.

As we saw in [Chapter 2](02-frequency-distributions.qmd), the smooth curve is a model for a distribution, not a literal record of observed frequencies. Within a suitably defined population, a variable such as height can often be approximately bell-shaped, with many observations near the middle and progressively fewer toward either extreme.

### The Normal Distribution Equation

The reason why we can describe the proportions of this kind of distribution so precisely is that it isn't just an arbitrary shape. It is a mathematically defined shape.

$$f(x) = \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\frac{(x-\mu)^2}{2\sigma^2}}$$

Now, you don't really have to know this equation for the work we're going to do throughout this book. So if it makes you queasy just looking at it, don't worry. But it is worth looking at briefly to get a sense of how it's constructed. The value $f(x)$ gives the height of the curve at a particular value of $x$. That height is called the **probability density**; it is not itself the probability of observing that exact value. Then we have $x-\mu$ squared, which should look familiar from our chapter on variability, and $\sigma$ is part of this too, so we know variability has something important to do with the shape. But again, don't worry about remembering the specifics. Just know that the equation defines the curve precisely, and areas under the curve will give us probabilities.
...
{{< interactive-figure zRegionsLabelled makeDistributionGraph options='{
  ...
  "shade": [
    {"to": -2, "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.12},
    {"between": [-2, -1], "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.22},
    {"between": [-1, 0], "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.38},
    {"between": [0, 1], "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.38},
    {"between": [1, 2], "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.22},
    {"from": 2, "label": "percent", "color": "var(--bc-accent, #2c7fb8)", "opacity": 0.12}
  ],
  "animate": true
