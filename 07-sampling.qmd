...
In the previous chapter, we used the normal distribution to determine the probability of seeing individual scores within a given range. But researchers aren't usually interested in single scores. They collect data from a sample, and use all the data from that sample to make probabilistic inferences about the population the sample belongs to. So here we'll discover how we can think about sampling and distributions of sample statistics in much the same way as we did within individual scores. The big ideas are sampling error and the central limit theorem.
...
The real version consists of a little box with a clear front with an arrangement of pegs and lots of little balls (originally lead shot) inside. When set upright, the balls bounce through the pegs as they fall from top to bottom. It's called a Quincunx (meaning five points, as in the arrangement of the pegs), or a Galton board, in honor of Francis Galton who developed it in the 1870s to demonstrate the normal distribution.^[Strictly speaking, it demonstrates the normal approximation of the binomial distribution, just like Abraham De Moivre did in Chapter 6.]

As Galton inscribed on the original device:

> The shot will all drop into the funnel, and running thence through its mouth, will pursue devious courses through the harrow and will accumulate in the vertical compartments at the bottom, there affording a representation of the law of dispersion.
...
Galton explained the point of the demonstration in more detail:

> The outline of the columns of shot that accumulate in the successive compartments approximate to the Curve of Frequency ... and is closely of the same shape however often the experiment is repeated. The outline of the columns would become nearly identical with the Normal Curve of Frequency, if the rows of pins were much more numerous, the shot smaller, and the compartments narrower; also if a larger quantity of shot was used.

This is really just our coin-flip and dice-rolling logic from the previous chapter taken further. A ball's final slot is determined by how many of its twelve bounces send it right rather than left. To end up in the far-left slot, a ball has to bounce left every single time, and there's only one way for that to happen, just like there's only one way to roll a total of two with two dice. To land in the middle, a ball needs an even mix of lefts and rights, and there are *many* different sequences of bounces that produce that mix, just like there are many ways to roll a total of seven. Any individual ball's path is unpredictable. But the pile that hundreds of balls build up together is, surprisingly, quite predictable.
...
This chapter is about what happens when we collect a bunch of observations and aggregate them into a single statistic, like the mean. The Quincunx is a nice visual metaphor for that process. Each ball is like a sample mean, and the pile of balls at the bottom is like the distribution of sample means.

An accumulation of chance events. We can think about each ball in the Quincunx as 'collecting' 12 observations as it makes its way through the 12 rows of pegs. Each time it meets a peg, there is a 50/50 chance it will bounce left or right, and each successive encounter is independent.

Let's think about it in the context of political polling. Say that, in reality, support for two candidates is exactly split, 50/50. If we take a sample of 12 people from the population and ask them which candidate they prefer, we might expect to find that half of the people in our sample support each candidate. But because of random sampling error, the proportion in our sample need not be exactly 50/50. In fact, there is a chance that we could randomly end up with 12 people who all support Candidate A. It's relatively unlikely, but not out of the question.

What if we had a sample of 100 people, rather than 12? In this case we'd expect something like 50 people to support each candidate, but it wouldn't be too surprising if we saw something like 60/40. It *would* be surprising if we happened to get 100 people who all support Candidate A, but it could happen. Now what if our sample size was 1,000? Or 1,000,000? Your intuition is probably telling you that the larger the sample, the more surprising an extremely lop-sided result would be.

Why is that, exactly? How can we describe the distribution of potential outcomes when we sample from a population and describe the sample using a statistic? The Quincunx is a nice visual metaphor for that process. Each ball is like a sample mean, and the pile of balls at the bottom is like the distribution of sample means.
...
For the examples that follow, we'll assume observations are **independent**: knowing one score doesn't change the probabilities for another. We'll return to what that requires in practice near the end of the chapter.
