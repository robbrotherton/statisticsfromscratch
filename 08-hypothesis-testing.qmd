...
Back in Chapter 5 we considered the possibility that a radioactive spider bite might have caused a difference in the reaction times of one individual. We supposed that following the bite, Peter Parker had a reaction time of 189 milliseconds on a test for which the general population had a mean of 284 milliseconds and a standard deviation of 50 milliseconds. We calculated his $z$-score to be $-1.9$. Assuming reaction times form a normal distribution, that score corresponds to a probability of less than three percent.

In other words, if Peter Parker's reaction time really came from the population of unbitten kids, we would only see a score this extreme about three times out of a hundred. People as fast as him are rare in the population, so it's correspondingly unlikely he'd just happen to be that fast by chance. The treatment, the spider bite, seemed to have caused a difference. His score looked a bit unusual, which we could take as evidence that he no longer belonged to that population.
...
So the situation facing us with a sample mean is slightly more complicated than the situation we faced with a single score. But only slightly! We have gather all the tools we need over the last few chapters.
...
First, let's just recalculate the $z$-score and probability of Peter Parker's single score, for old time's sake. Remember our imaginary test spiderman had a reaction time of 189 milliseconds on a test for which the general population had a mean of 284 milliseconds and a standard deviation of 50 milliseconds. We plug the numbers into the $z$-score equation and we get $-1.9$. 

$$ z = \frac{X - \mu}{\sigma} = \frac{189 - 284}{50} = -1.9 $$

We can look up $z = 1.9$ in the unit normal table to find a probability of less than .03. Pretty unlikely. $p(z \le -1.9) = .0287$. So if Peter Parker's reaction time really came from the population of unbitten kids, we would only see a score this extreme about three times out of a hundred.
...
Now our score calculation starts with that sample mean. We subtract the mean of the sampling distribution, which we know from the Central Limit Theorem is equal to the population mean, which we know from the website where you did that reaction time test is 284 milliseconds. So actually our numerator here is completely unchanged. It's only the denominator that changes. We divide by the standard error of the mean — the variability of the sampling distribution — rather than the variability of the population. The standard error is $\sigma$ over the square root of sample size. So 50, the population standard deviation, over the square root of 3, our sample size for this example. And now we get a $z$-score of $-3.29$, which is quite a bit lower than it was for Peter Parker alone. That's because you're inherently less likely to get extreme sample means than extreme single scores. The larger the sample, the more uncommon extreme means will be. Remember, the chances of one individual happening to have such an extreme score was less than three percent. The chances of three people all having an average score this extreme is correspondingly less likely.

And if we look up that $z$-score in the unit normal table, we get a much smaller $p$-value: $p(z \le -3.29) = .0005$.
...
And so we used our knowledge of the Central Limit Theorem and probability to determine what kind of sample means would be common or uncommon, assuming that there was no effect on reaction times. We collected some data — our hypothetical sample of three spider people — and we compared the $z$-score for their sample mean to the sampling distribution we would expect a completely unchanged population to produce. And we decided that the evidence was sufficiently unlikely if spider bites had no effect. So maybe they did.
...
But whatever our hypotheses, we next determine our decision criteria. We know that the $z$-test is appropriate here because I'm telling you we know the original population mean and standard deviation. And we know we'll be dealing with a normally distributed sampling distribution, because back when we first did this reaction time test, we said for the sake of argument that everyone's scores form a normally distributed population, which, like I said at the time, isn't strictly true for reaction times. But let's not worry about that. So we can set $\alpha$ at .05 if we like, giving us critical regions of .025, or two and a half percent, at either end of the distribution. Then we can find the corresponding $z$-score cutoffs in the unit normal table, and we find that that corresponds to $z$-scores of $\pm 1.96$.
