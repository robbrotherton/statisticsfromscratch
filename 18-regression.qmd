...
{{< interactive-figure regressionCover makeAssociationCover options='{"regression":true}' fallback="Multicolored dots with a correlation of about 0.6 pop into view, then their line of best fit draws across them as their colors change to green near the line through vermilion at the largest absolute residuals." >}}
...
Step two is to find the critical value for the test statistic. And since we're using an $F$-ratio, we use the $F$ table. The degrees of freedom for the numerator, $df_{regression}$, is one here. And for the denominator, $df_{residual}$ is $5 - 2$, which is three.
